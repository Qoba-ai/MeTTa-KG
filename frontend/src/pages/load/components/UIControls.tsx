...
const UIControls: Component<UIControlsProps> = (props) => {
  ...
  const baseButtonClass = "w-full text-left text-xs font-medium px-3 py-2 border border-border rounded transition-colors duration-200 disabled:opacity-60 disabled:cursor-not-allowed disabled:bg-muted";
  const activeButtonClass = "bg-primary border-primary text-primary-foreground";
  const defaultButtonClass = "bg-background hover:bg-accent hover:border-primary";

  return (
    <div class="flex flex-col gap-4 text-sm">
      {/* Layout Controls */}
      <div class="space-y-3 border-b border-border pb-3">
        <h3 class="text-xs font-semibold uppercase tracking-wider text-foreground">Layout</h3>
          ...
          <div class="space-y-3 rounded-md border border-border bg-muted/50 p-3">
            ...
            <div class="space-y-1">
              <div class="flex justify-between items-center">
                <label class="text-[11px] font-semibold uppercase text-muted-foreground">Animation (ms)</label>
                <span class="text-xs font-medium text-foreground">{layoutOptions().animationDuration || 1500}ms</span>
              </div>
              <input 
        ...
        <Show when={props.layoutState.isAnimating}>
          <div class="flex items-center gap-2 pt-1">
            ...
            <span class="min-w-[35px] text-right text-xs font-semibold text-foreground">{Math.round(props.layoutState.progress * 100)}%</span>
          </div>
        </Show>
      </div>
      ...
      {/* Display Controls */}
      <div class="space-y-3 border-b border-border pb-3">
        <h3 class="text-xs font-semibold uppercase tracking-wider text-foreground">Display</h3>
      ...
      {/* Export Controls */}
      <div class="space-y-3">
        <h3 class="text-xs font-semibold uppercase tracking-wider text-foreground">Export</h3>
        <div class="flex gap-2">
          <button onClick={props.onExportPDF} class={`${baseButtonClass} ${defaultButtonClass} text-center`}>PDF</button>
          <button onClick={props.onExportPNG} class={`${baseButtonClass} ${defaultButtonClass} text-center`}>PNG</button>
        </div>
      </div>
    </div>
  );
