...
import { fetchTokens, refreshCodes, deleteTokens, deleteToken } from '~/lib/api'
...
export const filteredAndSortedTokens = createMemo(() => {
    ...
    return tokens()
        .filter(
            (t) => nsRegex.test(t.namespace) && descRegex.test(t.description)
        )
        .sort((a, b) => {
            let result = 0
            switch (sortColumn()) {
...
export const handleDelete = async () => {
    ...
    try {
        ...
        mutateTokens(
            (current) =>
                current?.filter((t) => !idsToDelete.includes(t.id)) || []
        )
