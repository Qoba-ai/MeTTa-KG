...
export default function NameSpace() {
    ...
    const discoverPaths = async () => {
        ...
        try {
            ...
            const currentPath = namespace().length <= 1 ? '/' : '/' + namespace().slice(1).join('/');
            ...
            for (const t of allTokens) {
                if (t.namespace.startsWith(currentPath) && t.namespace !== currentPath) {
                    descendantPaths.add(t.namespace);
                }
            }
            
            descendantPaths.forEach(path => {
                const relativePath = path.substring(currentPath.length).replace(/^\//, '');
            ...
            const flatten = (node: Map<string, any>, path: string[], parentPrefix: string) => {
    ...
    return (
        <>
            ...
            <CommandDialog open={isExploring()} onOpenChange={setIsExploring}>
                ...
                <CommandList>
                    <Show when={!isLoading()} fallback={<CommandEmpty>Loading spaces...</CommandEmpty>}>
